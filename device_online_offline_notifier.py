    ...
    try:
        ...
        for device in devices:
            ...
            if previous_state != current_state:  # state changed
                # Initialize notifications counter if first time
                if devid not in device_notifications:
                    device_notifications[devid] = 0

                # Only send notification if less than 2
                if device_notifications[devid] < 2:
                    ...
                    device_notifications[devid] += 1
                else:
                    print(f"⚠️ {devnm} state changed but notification limit reached.")
                    device_status[devid] = current_state  # still update state
            else:
